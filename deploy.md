...
4. **Wait for receipt.** The page polls until the receipt arrives, then shows the deployed address and block number in the success message. **Copy these values immediately** — they are held in memory only. Refreshing the page wipes them; you would then need to look the address up on BSCscan or in your wallet history.
5. **Hardcode the address.** Edit [`build/assets/governance.js`](build/assets/governance.js), find `CONTRACT: '',` and `DEPLOY_BLOCK: 0,` in the `GOV` config, set them to the deployed address and block number. Commit and push.
...
- **No localStorage / sessionStorage / cookies.** The hardcoded `CONTRACT` constant in `governance.js` is the sole source of truth for the contract address. The page never reads or writes browser storage. Same-origin storage would be an attack vector — anything that can write storage on this origin (XSS in another page, malicious extension, shared/stolen browser) could swap in a drainer-contract address that the user would then unwittingly approve AHWA to and stake into. The trade-off: after a fresh deploy, refreshing the page before hardcoding the address loses it (recoverable via BSCscan or wallet tx history). Worth it.
